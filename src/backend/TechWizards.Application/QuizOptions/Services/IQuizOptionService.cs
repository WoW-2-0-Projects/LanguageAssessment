...
namespace TechWizards.Application.QuizOptions.Services;

/// <summary>
/// Defines the service operations for QuizOption entities.
/// </summary>
public interface IQuizOptionService
{
    ...
    /// <summary>
    /// Checks if a QuizOption exists by its unique identifier.
    /// </summary>
    /// <param name="quizOptionId">The unique identifier of the QuizOption to check.</param>
    /// <param name="cancellationToken">A token to cancel the operation if needed.</param>
    /// <returns>True if the QuizOption exists, otherwise false.</returns>
    ValueTask<bool> CheckByIdAsync(Guid quizOptionId, CancellationToken cancellationToken = default);
