...
namespace TechWizards.Infrastructure.ListeningAssessments.CommandHandlers;

/// <summary>
/// Handles the execution of the <see cref="SubmitListeningAssessmentCommand"/>.
/// </summary>
public class SubmitListingAssessmentCommandHandler(
    IQuizAssessmentSessionService sessionService,
    IListeningAssessmentService listeningAssessmentService)
    : ICommandHandler<SubmitListeningAssessmentCommand>
{
    public async Task Handle(SubmitListeningAssessmentCommand request, CancellationToken cancellationToken)
    {
        var assessment = await listeningAssessmentService
        ...
        await listeningAssessmentService.UpdateAsync(assessment, cancellationToken: cancellationToken);
